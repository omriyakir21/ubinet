...
if __name__ == "__main__":
    ...
    patches_dict_folder_path = os.path.join(
        paths.patches_dicts_path, f'{training_name}')
    os.makedirs(patches_dict_folder_path, exist_ok=True)
    data_for_training_folder_path = os.path.join(
        paths.patch_to_score_data_for_training_path, f'{training_name}')
    ...
    percentile_90_path = os.path.join(
    ...
    if plan_dict['create_patches']:
        create_patches(all_predictions, patches_dict_folder_path,
                       percentile_90, with_pesto, patch_index=int(sys.argv[1]))

    # MERGE THE PATCHES AFTER CREATING THEM
    merged_dict_path = os.path.join(
        patches_dict_folder_path, 'merged_protein_objects.pkl')
    ...
    if plan_dict['fetch_and_partition']:
        ...
        patch_to_score_model_path = os.path.join(
            paths.patch_to_score_model_path, f'{training_name}')
        scalers_folder_path = os.path.join(
            patch_to_score_model_path, 'scalers')
        ...
        partition_utils.partition_to_folds_and_save(
            sequences, data_for_training_folder_path)
