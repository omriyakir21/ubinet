...
def create_patches(all_predictions,dir_path,percentile_90, with_pesto: bool):
    i = int(sys.argv[1])
    PLDDT_THRESHOLD = 50

    create_patches_dict(i, dir_path, PLDDT_THRESHOLD, all_predictions,percentile_90, with_pesto)
...
def main(all_predictions_path: str, save_dir_path: str, with_pesto: bool):
   ...
   :rtype: List[Protein]
    ...
    merged_dict_path = os.path.join(save_dir_path, 'merged_protein_objects.pkl')
    ...
    if not os.path.exists(percentile_90_path):
        ...
        save_as_pickle(percentile_90, os.path.join(save_dir_path, 'percentile_90.pkl'))
    else:
    ...
    create_patches(all_predictions,save_dir_path,percentile_90, with_pesto)   
