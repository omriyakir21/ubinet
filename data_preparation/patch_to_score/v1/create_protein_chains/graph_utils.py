...
from data_preparation.ScanNet.db_creation_scanNet_utils import aa_out_of_chain
...
def c_alpha_distance(atom1: Atom, atom2: Atom) -> float:
    ...
    distance = np.sqrt(
        ((vector2[np.newaxis] - vector1[np.newaxis]) ** 2).sum(-1))
    return distance
...
def create_nodes_for_graph(residues: List[Residue],
    ...
    if len(residues) != len(scannet_ubiquitin_scores):
        raise SizeDifferentiationException()
...
def create_graph(scannet_ubiquitin_scores: np.array,
                 plddt_threshold: float, percentile_90: float,
                 structure: Structure) -> nx.Graph:
    graph = nx.Graph()
    model = structure.child_list[0]
    assert (len(model) == 1)
    for chain in model:
        residues = aa_out_of_chain(chain)
        nodes = create_nodes_for_graph(residues, scannet_ubiquitin_scores, plddt_threshold, percentile_90)
